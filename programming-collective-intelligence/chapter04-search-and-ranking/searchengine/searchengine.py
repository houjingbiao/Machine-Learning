...
#construct a word list whose words will be ignored in the processing
        ...
        for i in range(iterations):
            ...
            for (urlid, ) in self.con.execute('select rowid from urllist'):
                ...
                for (linker, ) in self.con.execute('select distinct fromid from link where toid = %d' % urlid):
                    linkingpr = self.con.execute('select score from pagerank where urlid=%d' % urlid).fetchone()[0]
...
class searcher:
    ...
    def distancescore(self, rows): # the version in the textbook is error
        # if there's only one word, everyone wins!
        if len(rows[0]) <= 2: return dict([(row[0], 1.0) for row in rows])

        mindistance = dict([(row[0], 1000000) for row in rows])

        for url in mindistance.keys():
            row = [rows[i][2] for i in range(len(rows)) if url==rows[i][0]]
