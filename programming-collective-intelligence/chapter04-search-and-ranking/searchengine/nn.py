...
class searchnet:
    ...
    def maketables(self):
        self.con.execute('create table wordlist(wordid, word)')
        self.con.execute('create table urllist(urlid, url)')
        self.con.execute('create table hiddennode(createkey)')
    ...
    def backPropagate(self, targets, N=0.5):
        ...
        hidden_deltas = [0.0]*len(len(self.hiddenids))
    ...
    def updatedatabase(self):
        ...
        for j in range(len(self.hiddenids)):
            for k in range(len(self.urlids)):
                self.setstrength(self.hiddenids[j], self.urlids[k], self.wo[j][k])
        self.con.commit()
